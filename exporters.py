...
class BaseExporter:
    ...
    def __init__(self, messages: List[Dict[str, Any]], id_index: Dict[str, Dict[str, Any]]):
...
class CSVExporter(BaseExporter):
    ...
    def export(self, output_path: Path) -> None:
        ...
        with open(output_path, 'w', newline='', encoding='utf-8') as f:
            writer = csv.DictWriter(f, fieldnames=['id', 'timestamp', 'sender', 'text', 'reply_to_id'])
            writer.writeheader()
...
class HTMLExporter(BaseExporter):
    ...
    def export(self, output_path: Path) -> None:
        ...
        for message in self.messages:
            ...
            html_lines.append(f'<div><span class="sender">{sender}</span><span class="timestamp">{timestamp}</span></div>')
...
class ExcelExporter(BaseExporter):
    ...
    def export(self, output_path: Path) -> None:
        ...
        try:
            ...
            import logging
            logging.warning("⚠️  openpyxl not installed. Install with: pip install openpyxl")
            return
        ...
        # Format header row
        header_fill = PatternFill(start_color='0084ff', end_color='0084ff', fill_type='solid')
